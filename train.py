...
def evaluate(simulator, device: str, flags):
    """Predict rollouts.
    ...
    # Output path
    if split=="test" not os.path.exists(flags["output_path"]):
...
def predict(device: str, FLAGS):
    ...
    simulator = _get_simulator(
        metadata, FLAGS.noise_std, FLAGS.noise_std, device, FLAGS.model_name
    )
...
def train(rank, flags, world_size):
    ...
    serial_simulator = _get_simulator(
        metadata, flags["noise_std"], flags["noise_std"], rank, flags["model_name"]
    )
    ...
    try:
        while not_reached_nsteps:
            ...
            for (position, particle_type, n_particles_per_example), labels in dl:
                ...
                non_kinematic_mask = (
                    (particle_type != KINEMATIC_PARTICLE_ID).clone().detach().to(rank)
                )
                ...
                if step % flags["nval_steps"] == 0 and rank == 0:
    ...
    if rank == 0:
        ...
        train_state = dict(
            optimizer_state=optimizer.state_dict(), global_train_state={"step": step}
        )
...
def _get_simulator(
    metadata: json, acc_noise_std: float, vel_noise_std: float, device: str, model: str
) -> learned_simulator.LearnedSimulator or learned_simulator.LearnedSimulator_locs:
    ...
    if model == "gns":
        ...
        simulator = learned_simulator.LearnedSimulator_locs(
            particle_dimensions=metadata["dim"],
            nnode_in=55 if metadata["dim"] == 3 else 30,
            nedge_in=2 * metadata["dim"] + 1 + 4,
            latent_dim=128,
            nmessage_passing_steps=10,
            nmlp_layers=2,
            mlp_hidden_dim=128,
